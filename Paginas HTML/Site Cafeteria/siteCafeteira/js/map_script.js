function init() {
	...
	adicionarMarker("Avenida Vinte e Três de Maio, São Paulo", map, mapnik,
			"red", "Loja 2");
}
...
function adicionarMarker(endereco, map, mapnik, color, nome) {
	...
	if (geocoder) {
		geocoder.geocode({
			...
			'region' : 'BR'
		}, function(results, status) { // Função Assíncrona
			if (status == google.maps.GeocoderStatus.OK) {

				var lat = results[0].geometry.location.lat();
				var lng = results[0].geometry.location.lng();
				var loc = new OpenLayers.LonLat(lng, lat).transform(
						'EPSG:4326', 'EPSG:3857');
				...
				switch (color) {
				case "red":
					marker.icon.imageDiv.firstChild.setAttribute('src',
							'img/marker.png');
					break;
				case "blue":
					marker.icon.imageDiv.firstChild.setAttribute('src',
